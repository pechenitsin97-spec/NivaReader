...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            statusText.text = "Карты ЭБУ загружены. Откройте лог."
        } catch (e: Exception) {
    ...
    private fun tryDecodeBoschPacket(line: String): String? {
        ...
        try {
            ...
            if (parts.size >= 3) {
                ...
                // 2. Пакет телеметрии 0001 (безопасный парсинг без падений)
                if (did == "0001" && parts.size > 20) {
                    try {
                        val p5 = parts[5].toIntOrNull(16) ?: 0
                        val p6 = parts[6].toIntOrNull(16) ?: 0
                        val rpm = ((p5 * 256) + p6) / 4

                        val tempRaw = parts[8].toIntOrNull(16) ?: 40
                        val coolant = tempRaw - 40

                        val tpsRaw = parts[12].toIntOrNull(16) ?: 0
                        val tps = (tpsRaw * 100) / 255

                        val voltRaw = parts[16].toIntOrNull(16) ?: 120
                        val vTotal = voltRaw * 10
                        val vMain = vTotal / 10
                        val vFrac = vTotal % 10

                        return "🔥 Обороты: $rpm об/мин  |  🌡 Антифриз: $coolant °C  |  ⚡ Дроссель: $tps%  |  🔋 АКБ: $vMain.$vFrac V"
                    } catch (e: Exception) {
                        return null
                    }
                }
